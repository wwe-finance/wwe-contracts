...
// File: contracts\WWELotteryNFT.sol

pragma solidity 0.6.12;
pragma experimental ABIEncoderV2;

contract WWELotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("WWE Lottery Ticket", "WWELT") {}
...
// 4 numbers
contract WWELottery is OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    IBEP20 public wweToken;
    // The Lottery NFT for tickets
    WWELotteryNFT public lotteryNFT;
    // adminAddress
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        wweToken.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward > 0) {
            safeWWETransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if (totalReward > 0) {
            safeWWETransfer(address(msg.sender), totalReward);
        }
    ...
    function safeWWETransfer(address _to, uint256 _amount) internal {
        uint256 wweBal = wweToken.balanceOf(address(this));
        if (_amount > wweBal) {
            wweToken.transfer(_to, wweBal);
        } else {
            wweToken.transfer(_to, _amount);
        }
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        wweToken.safeTransfer(address(msg.sender), _amount);
